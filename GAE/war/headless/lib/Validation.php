...
class Validation
{
	...
	function validate ($fields, $rules)
	{
		...
		for ($i = 0; $i < count($rules); $i++)
		{
			...
			while (preg_match("/^if:/", $row[0]))
			{
				...
				else array_shift($row); // remove this if-condition from line, and continue validating line
				}
			...
			else $error_message = $row[2]; // everything else!
			...
			{
				case "required":
					if (!isset($fields[$field_name]) || $fields[$field_name] == "") $errors[] = $field_name.':'.$error_message;
					break;

				case "digits_only":
					if (isset($fields[$field_name]) && preg_match("/\D/", $fields[$field_name])) $errors[] = $field_name.':'.$error_message;
					break;

				case "letters_only":
					if (isset($fields[$field_name]) && preg_match("/[^a-zA-Z]/", $fields[$field_name])) $errors[] = $field_name.':'.$error_message;
					break;

					// doesn't fail if field is empty
					case "valid_email":
					$regexp = "/^[a-z0-9]+([_+\\.-][a-z0-9]+)*@([a-z0-9]+([\.-][a-z0-9]+)*)+\\.[a-z]{2,}$/i";
					if (isset($fields[$field_name]) && !empty($fields[$field_name]) && !preg_match($regexp, $fields[$field_name])) $errors[] = $field_name.':'.$error_message;
					break;
				...
				case "length":
					...
					switch ($comparison_rule)
					{
						case "greater_than_or_equal":
							if (!(strlen($fields[$field_name]) >= $rule_string)) $errors[] = $field_name.':'.$error_message;
							break;
						...
						case "equal":
							...
							if (preg_match("/-/", $rule_string))
							{
								...
								if (strlen($fields[$field_name]) < $start || strlen($fields[$field_name]) > $end) $errors[] = $field_name.':'.$error_message;
							}
				...
				case "range":
					...
					{
						case "greater_than":
							if (!($fields[$field_name] > $rule_string)) $errors[] = $field_name.':'.$error_message;
							break;
						case "less_than":
							if (!($fields[$field_name] < $rule_string)) $errors[] = $field_name.':'.$error_message;
							break;
						...
						case "equal":
							...
							if (($fields[$field_name] < $start) || ($fields[$field_name] > $end)) $errors[] = $field_name.':'.$error_message;
							break;
					}
					break;

				case "same_as":
					if ($fields[$field_name] != $fields[$field_name2]) $errors[] = $field_name.':'.$error_message;
					break;
				...
				case "valid_date":
					...
					$is_later_date = false;
					if ($date_flag == "later_date") $is_later_date = true;
					else if ($date_flag == "any_date") $is_later_date = false;

					if (!$this->is_valid_date($fields[$field_name], $fields[$field_name2], $fields[$field_name3], $is_later_date)) $errors[] = $field_name.':'.$error_message;
					break;

				case "is_alpha":
					if (preg_match('/[^A-Za-z0-9]/', $fields[$field_name])) $errors[] = $field_name.':'.$error_message;
					break;
				...
				case "custom_alpha":
					...
					{
						if (array_key_exists($field_name2[$j], $chars)) $reg_exp_str .= $chars[$field_name2[$j]];
						else $reg_exp_str .= $field_name2[$j];
					}

					if (!empty($fields[$field_name]) && !preg_match("/$reg_exp_str/", $fields[$field_name])) $errors[] = $field_name.':'.$error_message;
					break;
				...
				case "reg_exp":
					...
					// rather crumby, but...
					if (count($row) == 5) $reg_exp = "/".$reg_exp_str."/".$row[3];
					else $reg_exp = "/".$reg_exp_str."/";

					if (!empty($fields[$field_name]) && !preg_match($reg_exp, $fields[$field_name])) $errors[] = $field_name.':'.$error_message;
					break;
	...
	function is_valid_date ($month, $day, $year, $is_later_date)
	{
		...
		else $days_in_month = array(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31);
